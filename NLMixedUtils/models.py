...
class ComplexVisNetwork(nn.Module):
    def __init__(self, 
                    in_features, 
                    N_cells, 
                    shift_in=3, 
                    ...
                    meanbias = None,
                    device='cuda'):
        super(ComplexVisNetwork, self).__init__()
        r''' Complex Visual GLM Network
        Args: 
            in_feature: size of the input dimension
            N_cells: the number of cells to fit
            shift_in: size of the input to shifter network. 
        ...
        self.Cell_NN = nn.ModuleList([nn.Linear(self.in_features//2, self.N_cells,bias=True),
        ...
        self.activations = nn.ModuleDict({'SoftPlus':nn.Softplus(),
                                          'ReLU': nn.ReLU(),})
        torch.nn.init.uniform_(self.Cell_NN[0].weight, a=-1e-6, b=1e-6)
        torch.nn.init.uniform_(self.Cell_NN[1].weight, a=-1e-6, b=1e-6)
